...
The RPM spec file provided here is for YellowDuckieCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://yellowduckie.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[YellowDuckieCoin git repository](https://github.com/YellowDuckieCoin/YellowDuckie) but are not part of
...
Currently only `Source10` is used, for the example `yellowduckiecoin.conf` file.
...
in the yellowduckiecoin git repository. At present only an SVG version of the YellowDuckieCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the yellowduckiecoin
...
needed when 0.12.1 is released, a proper fix is already in the YellowDuckieCoin git
master branch.
...
idea to build YellowDuckieCoin Core against a static version of that library compiled
...
The `yellowduckiecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `yellowduckiecoin-server` package. At this point, most RPM based
...
The `yellowduckiecoin-server` package should have SELinux support. How to properly do
...
YellowDuckieCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/yellowduckiecoin/). He has been
packaging YellowDuckieCoin for Fedora at least since 2012.
...
4. I renamed the `yellowduckiecoin` package that contains the Qt GUI to `yellowduckiecoin-core` as
that appears to be how the general population refers to it, in contrast to
`yellowduckiecoin-xt` or `yellowduckiecoin-classic`. I wanted to make sure the general population
