...
# Copyright (c) 2017-2021 The YellowDuckieCoin Core developers
...
`test/functional/test_framework/test_framework.py:YellowDuckieCoinTestFramework.main`.
...
BASE_SCRIPTS= [
    ...
    'rpc_net.py',
    'rpc_misc.py',
    'interface_yellowduckiecoin_cli.py',
...
def main():
    ...
    tmpdir = "%s/yellowduckiecoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and yellowduckiecoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_yellowduckiecoind = config["components"].getboolean("ENABLE_YellowDuckieCoinD")
    if not (enable_wallet and enable_cli and enable_yellowduckiecoind):
        print("No functional tests to run. Wallet, utils, and yellowduckiecoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if yellowduckiecoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "yellowduckiecoind"]) is not None:
            print("%sWARNING!%s There is already a yellowduckiecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "YellowDuckieCoinD" not in os.environ:
        os.environ["YellowDuckieCoinD"] = build_dir + '/src/yellowduckiecoind' + exeext
        os.environ["YellowDuckieCoinCLI"] = build_dir + '/src/yellowduckiecoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `yellowduckiecoin-cli help` (`rpc_interface.txt`).
