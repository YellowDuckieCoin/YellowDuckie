 ...
 * between yellowduckiecoind, and yellowduckiecoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("yellowduckiecoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateYellowDuckieCoins(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), YellowDuckieCoin_CONF_FILENAME));
    if (mode == HMM_YellowDuckieCoinD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), YellowDuckieCoin_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/YellowDuckieCoin/YellowDuckie>";
    const std::string URL_WEBSITE = "<https://yellowduckie.net>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("yellowduckiecoin-loadblk");
...
/** Sanity checks
 *  Ensure that YellowDuckieCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("YellowDuckieCoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single YellowDuckieCoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", YellowDuckieCoin_CONF_FILENAME)).string());
    ...
    GenerateYellowDuckieCoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
