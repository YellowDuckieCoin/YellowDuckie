...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** YD START */
    case OP_YD_ASSET              : return "OP_YD_ASSET";
    /** YD END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_YD_ASSET) { // OP_YD_ASSET is always in the 25 index
            int index = -1;
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == YD_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == YD_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == YD_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == YD_R) {
...
{
    // 1 OP_YD_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_YD_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_YD_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_YD_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_YD_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
