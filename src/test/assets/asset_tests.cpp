...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of YellowDuckieCoin NOT allowed
        BOOST_CHECK(!IsAssetNameValid("YD", type));
        BOOST_CHECK(!IsAssetNameValid("YellowDuckieCoin", type));
        BOOST_CHECK(!IsAssetNameValid("YellowDuckieCoin", type));

        //- Versions of YellowDuckieCoin ALLOWED
        BOOST_CHECK(IsAssetNameValid("YellowDuckieCoin.COIN", type));
        BOOST_CHECK(IsAssetNameValid("YellowDuckieCoin_COIN", type));
        BOOST_CHECK(IsAssetNameValid("YDSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERYD", type));
        BOOST_CHECK(IsAssetNameValid("YellowDuckieCoinSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEYellowDuckieCoin", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_YellowDuckieCoinS", type));
        BOOST_CHECK(IsAssetNameValid("SEYDOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("YellowDuckieCoin", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("YellowDuckieCoin", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
