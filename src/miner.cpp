...
// Block rewards
    ...
    if(nHeight==1){
        minerSubsidy=nSubsidy-10500000000*COIN;//for dev  invest and community
        ...
        if (!IsValidDestination(destDevAutonomous)) {
            LogPrintf("IsValidDestination: Invalid YD Coin DEV address %s \n", dev_address);
        }
        ...
        coinbaseTx.vout[1].nValue=10500000000*COIN;
    }
    else{
...
void static YellowDuckieCoinMiner(const CChainParams& chainparams)
{
    LogPrintf("YellowDuckieCoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yellowduckiecoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("YellowDuckieCoinMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("YellowDuckieCoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("YellowDuckieCoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("YellowDuckieCoinMiner:\n  proof-of-work found\n  hash: %s\n  target: %s\n", hash.GetHex(), hashTarget.GetHex());
    ...
    {
        LogPrintf("YellowDuckieCoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("YellowDuckieCoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateYellowDuckieCoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&YellowDuckieCoinMiner, boost::cref(chainparams)));
    }
