...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for YD
    ...
    if (p_rewardSnapshot.strDistributionAsset != "YD") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is YD with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using YD differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "YD") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse YellowDuckieCoin address (already validated during ownership snapshot creation)
