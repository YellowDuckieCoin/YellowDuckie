...
#include "yellowduckiecoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a YellowDuckieCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new YellowDuckieCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new YellowDuckieCoinAddressCheckValidator(parent));
}
...
bool parseYellowDuckieCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no yellowduckiecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("yellowduckiecoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!YellowDuckieCoinUnits::parse(YellowDuckieCoinUnits::YD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseYellowDuckieCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert yellowduckiecoin:// to yellowduckiecoin:
    //
    //    Cannot handle this later, because yellowduckiecoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("yellowduckiecoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "yellowduckiecoin:");
    }
    QUrl uriInstance(uri);
    return parseYellowDuckieCoinURI(uriInstance, out);
}

QString formatYellowDuckieCoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("yellowduckiecoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(YellowDuckieCoinUnits::format(YellowDuckieCoinUnits::YD, info.amount, false, YellowDuckieCoinUnits::separatorNever));
        paramCount++;
    }
...
bool openYellowDuckieCoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(YellowDuckieCoin_CONF_FILENAME);
    ...
    /* Open yellowduckiecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "YellowDuckieCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "YellowDuckieCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("YellowDuckieCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for YellowDuckieCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "yellowduckiecoin.desktop";
    return GetAutostartDir() / strprintf("yellowduckiecoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a yellowduckiecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=YellowDuckieCoin\n";
        else
            optionFile << strprintf("Name=YellowDuckieCoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the yellowduckiecoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef yellowduckiecoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (yellowduckiecoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, yellowduckiecoinAppUrl);

    CFRelease(yellowduckiecoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add yellowduckiecoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, yellowduckiecoinAppUrl, nullptr, nullptr);
    }
