...
#include "yellowduckiecoingui.h"

#include "yellowduckiecoinunits.h"
...
const std::string YellowDuckieCoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString YellowDuckieCoinGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "YDBTC"  , 1,          8},
    { "mBTC",   "YDBTC"  , 1000,       5},
    { "µBTC",   "YDBTC"  , 1000000,    2},
    { "Satoshi","YDBTC"  , 100000000,  0},
    { "USDT",   "YDUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(YellowDuckieCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

YellowDuckieCoinGUI::YellowDuckieCoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
YellowDuckieCoinGUI::~YellowDuckieCoinGUI()
{
...
void YellowDuckieCoinGUI::loadFonts()
{
...
void YellowDuckieCoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a YellowDuckieCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and yellowduckiecoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to YD addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your YellowDuckieCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified YellowDuckieCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a yellowduckiecoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible YellowDuckieCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void YellowDuckieCoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void YellowDuckieCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        // if(IconsOnly) {
        //     labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/ydtext")));
        // }
        // else {
        //     labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/yellowduckiecointext")));
        // }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* yellowduckiecoinLabelLayout = new QVBoxLayout(toolbarWidget);
        yellowduckiecoinLabelLayout->addWidget(labelToolbar);
        yellowduckiecoinLabelLayout->addWidget(m_toolbar);
        yellowduckiecoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        yellowduckiecoinLabelLayout->addStretch(1);
        ...
        // labelCurrentMarket->setText(tr("YellowDuckieCoin Market Price"));
        ...
        comboYDUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboYDUnit->addItems(list);
        comboYDUnit->setFixedHeight(26);
        comboYDUnit->setContentsMargins(5,0,0,0);
        comboYDUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboYDUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/YellowDuckieCoin/YellowDuckie/releases\">New Wallet Version Available</a>");
        ...
        // priceLayout->addWidget(comboYDUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboYDUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest YellowDuckieCoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/YellowDuckieCoin/YellowDuckie/releases";
...
void YellowDuckieCoinGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    // if(IconsOnly) {
    //     labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/ydtext")));
    //     m_toolbar->setMaximumWidth(65);
...
void YellowDuckieCoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool YellowDuckieCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool YellowDuckieCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void YellowDuckieCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void YellowDuckieCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void YellowDuckieCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void YellowDuckieCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void YellowDuckieCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void YellowDuckieCoinGUI::optionsClicked()
{
...
void YellowDuckieCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void YellowDuckieCoinGUI::showDebugWindow()
{
...
void YellowDuckieCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void YellowDuckieCoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void YellowDuckieCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void YellowDuckieCoinGUI::openClicked()
{
...
void YellowDuckieCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void YellowDuckieCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void YellowDuckieCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void YellowDuckieCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void YellowDuckieCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void YellowDuckieCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** YD START */
void YellowDuckieCoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void YellowDuckieCoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void YellowDuckieCoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void YellowDuckieCoinGUI::gotoRestrictedAssetsPage()
{
...
void YellowDuckieCoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to YellowDuckieCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void YellowDuckieCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void YellowDuckieCoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void YellowDuckieCoinGUI::updateHeadersSyncProgressLabel()
{
...
void YellowDuckieCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void YellowDuckieCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("YellowDuckieCoin"); // default title
    ...
    // Append title to "YellowDuckieCoin - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void YellowDuckieCoinGUI::changeEvent(QEvent *e)
{
...
void YellowDuckieCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void YellowDuckieCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void YellowDuckieCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "YD")
        msg += tr("Amount: %1\n").arg(YellowDuckieCoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(YellowDuckieCoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void YellowDuckieCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void YellowDuckieCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool YellowDuckieCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool YellowDuckieCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void YellowDuckieCoinGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void YellowDuckieCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void YellowDuckieCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void YellowDuckieCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void YellowDuckieCoinGUI::detectShutdown()
{
...
void YellowDuckieCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void YellowDuckieCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void YellowDuckieCoinGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(YellowDuckieCoinGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void YellowDuckieCoinGUI::subscribeToCoreSignals()
{
...
void YellowDuckieCoinGUI::unsubscribeFromCoreSignals()
{
...
void YellowDuckieCoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void YellowDuckieCoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<YellowDuckieCoinUnits::Unit> units = YellowDuckieCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const YellowDuckieCoinUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(YellowDuckieCoinUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(YellowDuckieCoinUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (YellowDuckieCoinUnits::Unit u : YellowDuckieCoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(YellowDuckieCoinUnits::name(u)), this);
...
{
    setText(YellowDuckieCoinUnits::name(newUnits));
}
...
void YellowDuckieCoinGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void YellowDuckieCoinGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "YellowDuckieCoinGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboYDUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void YellowDuckieCoinGUI::getPriceInfo()
{
...
#ifdef ENABLE_WALLET
void YellowDuckieCoinGUI::mnemonic()
{
...
void YellowDuckieCoinGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/YellowDuckieCoinProject/YellowDuckieCoin/releases"));
