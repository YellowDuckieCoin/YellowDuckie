...
The YellowDuckieCoin Core daemon can be configured to act as a trusted "border
router", implementing the yellowduckiecoin wire protocol and relay, making
...
The ZeroMQ feature in YellowDuckieCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building yellowduckiecoind:
...
For instance:
    ...
    $ yellowduckiecoind -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/yellowduckiecoind.tx.raw
...
These options can also be provided in yellowduckiecoin.conf.
...
From the perspective of yellowduckiecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into yellowduckiecoind directly. Furthermore, no information is
...
using. YellowDuckieCoind appends an up-counting sequence number to each
